...
\begin{paragraf*}
This chapter consists of the heart of the thesis. In fact, carefully studied,
this list of examples could probably make the proofs chapter obsolete.

Every example comes with a label of the form $(\mt{exXYZ})$, which refers its a Macaulay2
test code. Those files can found at
\begin{center}
...
\[
	...
	\bgroup\begin{bmatrix}0&
      ...
      \end{bmatrix}\egroup
\]
whose determinant is $P(\mathbf x)$.
...
Note that the claim that $\phi$ is birational follows, a fortiori, from the degree formula
\eqref{eq:degree-formula}. Indeed, we have
\[
	4\deg(\phi)=2^2-0
\]
We confirm this using Proposition \ref{prop:deg-GB} in Example \ref{ex310}.
\end{example}

\begin{example}[$\mt{ex302}$]
\label{ex302}
Clearly, if we replace $s,t,u$ in Example \ref{ex301} by general linear forms $L_0,L_1,L_2$,
effectively changing coordinates on the source, we get the same answer.
...
\[
	...
	{x}_{2}^{2}-{x}_{0} {x}_{1} {x}_{2} {x}_{3})
\]
and $\phi$ is again basepoint-free, basically because the $L_k$ are three general forms on
a 2-dimensional variety.
However, the self-intersection of $\O(2,2)$ is now $8$.
...
For $\d=(2,1)$ we get a square matrix of size $6$ with $h=(4,2)$ for the which the
last equality applies.
For $\d=(2,2)$ we get a $9\times12$ matrix with $h=(11,1)$.
\end{example}
...
Indeed, on the affine local where $u=v=1$, the point looks like $V(st,s^2+t^2)$.
The equation of the image remains the same.
Once again, \eqref{eq:degree-formula} we know that $\phi$ must be birational.
...
Then $\phi$ is birational with three basepoints --- $(1,0,0)$, $(0,1,0)$ and $(0,0,1)$,
...
In light of Theorem \ref{th:rad-minors}, we can carry out the same calculations
even though the image is of codimension strictly bigger than $1$.
...
\[
	\det(N)=P(\mathbf x)^2
\]
where one should note the principal $2$-minor is just $P(\mathbf x)$.
...
\label{ex309}
Let $N_1$ be the matrix of linear columns for $\d=1$ in Example \ref{ex305}.
...
that is, except the one corresponding to the syzygy $s^{4} {x}_{1}{x}_{2}-s^{3} t {x}_{3}^{2}$.
Then $\det(M)=0$.
...
Indeed, $\mt{multiplicity}(J)$ gives $45$.

Let $Q_k$ be the prime ideals corresponding to the points $q_k$, and let $J_k=J:{Q_k}^\infty$.
The the individual degree and multiplicity can be computed by computing the degree and multiplicity
of the corresponding ideals $J_k$. Those capture the local structure of $Z$ near $q_k$.
We have
\[
	\begin{cases}
...
Suppose further that $\text{Sing}(Y)$, the singular locus $Y$,
...
\end{bmatrix}
\]
where the entries are scaled variables.
\end{example}
...
\label{ex314}
We apply Proposition \ref{prop:deg-GB} to compute the degree of the rational map in Example \ref{ex301}.
We have
\[
	I=\Bigg\la
	\begin{cases}
...
in that case the singular locus is all of $Y$. This can be seen in Example \ref{ex308}.
